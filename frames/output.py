...
class OutputFrame(ctk.CTkFrame):
    ...
    def create_back_save_frame(self, master, **kwargs):
        ...
        # back button
        self.output_back_button = ctk.CTkButton(self.output_back_save_frame, text='Back', anchor='center', bg_color='transparent',
                                              image=master.back_image, compound='left',
                                              font=ctk.CTkFont(size=semi_header, weight='bold'),
                                              command=master.preference_button_callback)
        self.output_back_button.grid(row=0, column=0, padx=10, pady=(10,10), sticky='s')

        # solve button
        self.output_home_button = ctk.CTkButton(self.output_back_save_frame, text='Home', anchor='center', bg_color='transparent',
                                              image=master.home_image_light, compound='left',
                                              font=ctk.CTkFont(size=semi_header, weight='bold'),
                                              command=master.home_button_callback)
        self.output_home_button.grid(row=0, column=1, padx=10, pady=(10,10), sticky='s')
